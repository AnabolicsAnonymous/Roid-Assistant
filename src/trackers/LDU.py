...
class LDU():
    ...
    def __init__(self, config):
        ...
        self.upload_url = 'https://www.theldu.net/api/torrents/upload'
        self.search_url = 'https://www.theldu.net/api/torrents/filter'
        self.signature = f"\n[center][url=https://github.com/AnabolicsAnonymous/Roid-Assistant][img=500]https://i.ibb.co/Nmnq5K6/roid.gif[/img][/url][/center]"
        self.banned_groups = [] # weird.
    ...
    async def upload(self, meta):
        ...
        if meta['debug'] == False:
            ...
            try:
                console.print(f"{get_date('info')}[bold green]Tracker response: {response.json()}")
            except:
                console.print(f"{get_date('info')}[bold green]It may have uploaded, go check")
                return 
        else:
            console.print(f"{get_date('info')}[cyan]Request Data: {data}")
        open_torrent.close()
